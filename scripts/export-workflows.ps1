...
docker compose --env-file .env -f docker-compose.yml exec -T n8n sh -c 'rm -rf /tmp/n8n-workflow-export && mkdir -p /tmp/n8n-workflow-export'
docker compose --env-file .env -f docker-compose.yml exec -T n8n n8n export:workflow --backup --output=/tmp/n8n-workflow-export
...
docker compose --env-file .env -f docker-compose.yml exec -T n8n rm -rf /tmp/n8n-workflow-export
